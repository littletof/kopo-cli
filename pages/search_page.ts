...
export class SearchPage {
    static async show(args: Args, options?: {searchTerm?: string, exact?: boolean, registries?: Registry[]}): Promise<void> {
        ...
        UI.cls();
        const searchIndicator = `Searching for [${Theme.accent(options.searchTerm)}]...`;
        console.log( searchIndicator + backspace(searchIndicator.length) + `\x1B[${1}A`);

        await this.showSearchResult(args, options as any);
    }

    static async showSearchResult(args: Args, options: {searchTerm: string, exact?: boolean, registries?: Registry[]}): Promise<void> {
        ...
        const exactMatches = (await Promise.all(registries.map(async registry => {
            const moduleInfo = await registry.getModuleInfo(options.searchTerm);
            if(!moduleInfo) {
                return undefined;
            }
        ...
        // option to searchterm -> browsepage with query
        ...
        const exactOpts = exactMatches.map(r => ({result: r, ...UI.selectListOption({
            name: `${r?.registry.getRegistryInfo().name} - ${r?.result.info?.name} @ ${r?.result.currentVersion}`,
            value: r?.registry.getRegistryInfo().key,
        })}));

        /* 
        if !exact -> registry selector -> browse with query
        if !exactMathces -> registries.length === 1 -> browseWithquery
        ...
        */
        ...
        const selected = await UI.selectList({
            message: `KOPO CLI - ${registries.length === 1? `${registries[0].getRegistryInfo().name} - ` : ''}Search for: ${Theme.accent(options.searchTerm)}`,
            options: [
                UI.listOptions.empty,
                ...finalOptions,
        ...
        if(selectedExact) {
            UI.cls();
            await ModulePage.show(args, {module: selectedExact.result?.result.info?.name!, registry: selectedExact.result?.registry!, showTitle: true});
            UI.cls();
